...
func mcpTool() mcp.Tool {
	return mcp.NewTool("shodan_ip_query",
		...
		mcp.WithString("tier",
			mcp.Description("Deprecated per-request override; accepted for compatibility but pacing uses SHODAN_TIER from server startup config")),
...
func ipBulkTool() mcp.Tool {
	return mcp.NewTool("shodan_ip_query_bulk",
		...
		mcp.WithString("tier",
			mcp.Description("Deprecated per-request override; accepted for compatibility but bulk pacing uses SHODAN_TIER from server startup config")),
		mcp.WithBoolean("clear_cache",
